...
라이브클래스 프로덕트 엔지니어 과제 A - 수강 신청 시스템 구현 프로젝트입니다.

## 프로젝트 개요

크리에이터가 강의를 개설하고, 클래스메이트가 강의에 수강 신청한 뒤 결제 확정을 통해 수강 상태를 변경하는 REST API 서버입니다.
...
- 인증/인가 시스템은 구현하지 않고, 요청 값의 `creatorId`, `memberId`, `role`을 신뢰하는 방식으로 단순화했습니다.
- 강의 등록은 `role` 값이 `CREATOR`인 경우에만 허용합니다.
- 강의 상태는 `DRAFT -> OPEN -> CLOSED` 흐름을 기준으로 구현했습니다.
- 수강 신청은 강의 상태가 `OPEN`인 경우에만 가능합니다.
- 현재 신청 인원은 `PENDING`, `CONFIRMED` 상태의 신청만 계산합니다.
- `CANCELLED` 상태의 신청은 정원 계산에서 제외합니다.
...
- Controller, Service, Repository, Entity 계층을 분리했습니다. 요청 처리, 비즈니스 규칙, 영속성 책임을 명확히 나누기 위함입니다.
- Repository는 Spring Data JPA 인터페이스 대신 `EntityManager` 기반 구현체를 사용했습니다. 비관적 락과 JPQL 조회 조건을 명시적으로 제어하기 위함입니다.
...
- 실제 회원 인증, 로그인, JWT, 세션 관리는 구현하지 않았습니다.
- 크리에이터 권한은 `role=CREATOR` 요청 값으로만 검증합니다.
- 강의 수정/삭제 API는 구현하지 않았습니다.
- 수강 취소 가능 기간 제한은 코드 주석만 있고 실제 검증 로직은 구현하지 않았습니다.
- 대기열(waitlist) 기능은 구현하지 않았습니다.
- 강의별 수강생 목록 조회 API는 구현하지 않았습니다.
- 신청 내역 페이지네이션은 구현하지 않았습니다.
- 결제 시스템 연동, 결제 금액 검증, 결제 실패 처리는 구현하지 않았습니다.
- `endDate`가 `startDate` 이후인지 검증하는 별도 도메인 로직은 없습니다.
- 운영 DB 설정은 없으며 기본 H2 인메모리 DB를 사용합니다.
- 목록 조회 API는 정렬 조건을 별도로 지정하지 않습니다.

## AI 활용 범위

- 과제 요구사항을 도메인, API, 비즈니스 규칙 단위로 정리하는 데 AI를 활용했습니다.
- README 문서 구조화, API 예시 작성, 구현 범위와 미구현 범위 정리에 AI를 활용했습니다.
- 구현 코드의 최종 판단과 프로젝트 맥락에 맞는 내용 검토는 개발자가 수행한다는 전제로 작성했습니다.
